        ...
        if(_.isArray(data)){
            ...
            for(var x=0;x<lngData;x++){
                for(var i=0;i<lngColumns;i++){
                    ...
                    if(col.formatter.outbound.length>0) data[x][col.field] = self.$restfulFormatter.$processFormatters(col.field,data[x][col.field],col.formatter.outbound);
                    if(data[x][col.field]===null || data[x][col.field]==="null") data[x][col.field] = "";
                }
            }
        }else{
            var lngColumns = obj.columns.length;
