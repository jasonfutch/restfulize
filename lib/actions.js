        ...
        if(_.isArray(data)){
            ...
            for(var x=0;x<lngData;x++){
                for(var i=0;i<lngColumns;i++){
                    ...
                    if(col.formatter.length>0) data[x][col.field] = self.$restfulFormatter.$processFormatters(col.field,data[x][col.field],col.formatter);
                }
            }
        }else{
            var lngColumns = obj.columns.length;
            for(var i=0;i<lngColumns;i++){
                var col = obj.columns[i];
                if(col.formatter.length>0) data[col.field] = self.$restfulFormatter.$processFormatters(col.field,data[col.field],col.formatter);
            }
        }
        return data;
    };
