...
var $sql = require('./../examples/documentation-api/mysql');
...
module.exports = function restfulActions(tables, req, res, errorHandler){
        ...
        return $q.Promise(function(resolve, reject, notify) {
            ...
            var buildComplete = function(orgData){
                ...
                for(item in obj.body){
                    if(obj.body.hasOwnProperty(item)){
                        ...
                        if(column!==undefined){
                            ...
                            var value = self.$processFields(column.actions, obj.body[item], item, obj.strict, obj._force);
                            if(value===false){
                        ...
                        if(column.actions.required===true){
                            if(column.actions.editable===false){
                                if(_.isUndefined(obj.body[field])){
                                    obj.body[field] = self.$processFields(table.fields[field].actions, "", field, obj.strict, obj._force, true);
                                    obj.fields.push({
                ...
                if(strUniqueDataField!=''){
                    self.$runUniqueDataFields(table,orgData,strUniqueDataField).then(function(){
                        resolve(obj);
                    }).fail(function(){
                        reject();
                    });
    ...
    this.$runUniqueDataFields = function(table,orgData,str){
        var where = str;
        if(_.isObject(orgData)) where = "("+str+" AND "+table.key+"!='"+orgData[table.key]+"')";

        var sqlString = new $sqlString();
        sqlString.limit('1');
        var strSQL = sqlString.select(table.name,where);
    ...
    this.$processUniqueDataFields = function(field,column,obj,str){
        ...
        for(var i=0;i<cntUniqueDataField;i++){
            ...
            strArgument += uniqueField+"='"+obj.body[uniqueField]+"'";
        }
        if(str!="") str += " OR ";
        str += "("+strArgument+")";
        return str;
    ...
    this.$processFields = function(action, value, field, bolStrict, force, bolPrivate){ //bolPrivate means the data is coming from internal and assumed safe
        var temp;
        ...
        if(action.editable===false && bolPrivate==false){
