...
//                reject();
            ...
            var buildComplete = function(orgData){
                ...
                for(var i=0;i<lngFields;i++) {
                    ...
                    if (column.actions.timestamp === true) {
                        body[field] = self.$processFields(table.fields[field].actions, "now()", field, obj.strict, type, obj._force, true);
                        obj.fields.push({
    ...
    this.$runCheckExistIn = function(ary,obj){
        ...
        $async.eachSeries(ary, function(o, callback) {
            ...
            var filters = restfulRequest.buildFilter(objCheck.filter,tableDef.fields,[{regex:/\{field_value\}/g,value:obj.body[o.field]+""}]);
            ...
            $sql.connect().then(function(connection){
                connection.execute(strSQL).then(function(rows) {
                    ...
                    if(rows.length>0){
                        ...
                        self.onError(409, 100, "Can not find corresponding record in table '"+objCheck.table+"' with "+o.field+"='"+obj.body[o.field]+"'.");
                        deferred.reject();
                    }
                });
            });
        ...
        $sql.connect().then(function(connection){
            connection.execute(strSQL).then(function(rows) {
                ...
                if(rows.length>0){
                    ...
                    self.onError(409, 100, 'Record is not unique in table '+strTable+' with '+str);
                    deferred.reject();
                }else{
                    deferred.resolve();
                }
    ...
    this.$processFields = function(action, value, field, bolStrict, type, force, bolPrivate){ //bolPrivate means the data is coming from internal and assumed safe
        ...
        if((action.required===true && value==='') || (action.required===true && value==='{}' && action.allowedEmptyObject===false)){
            if(action.default===''){
