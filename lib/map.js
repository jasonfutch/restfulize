...
module.exports = function restfulMap(req,res){
    ...
    this.build = function(map){
        ...
        var runTask = function(json){
            ...
            task = $extend(true,obj,map[currentID]);
            ...
            task.toComplete = function(resp,o){
                if(currentID===0){
                    ...
                    json[task.section] = onComplete(resp,o);
                }
                currentID++;
                runTask(json);
            };
    ...
    this.$action = function(task){
        switch((task.type).toLowerCase()){
            case 'get':
                var restfulGet = new $restfulGet(req,res);
                restfulGet.build(task.tables,task.overwrites,task.defaults,task.toComplete);
                break;
            case 'list':
                var restfulList = new $restfulList(req,res);
                restfulList.build(task.tables,task.overwrites,task.defaults,task.toComplete);
                break;
        }
    };
