...
var $setData = function(dir,bolForceRun){
    var tables = {};
    var objects = {};
    if(!dir) dir = $path.join(__dirname, '/../../../data/schemas/');
    var dirObject = $path.join(__dirname, '/../../../data/objects/');
    ...
    if(prevDir!==dir || bolForceRun) {
        ...
        $fs.readdirSync(dirObject).forEach(function (file) {
            try {
                var fN = file.replace(/\.[^/.]+$/, "");
