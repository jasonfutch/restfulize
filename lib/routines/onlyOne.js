...
module.exports = function(){
    ...
    return {
        ...
        onComplete: function(objBuild,obj){
            console.log('onlyOne: onComplete');
            var deferred = $q.defer();
                        ...
                        connection.execute(strSQL).then(function(rows) {
                            ...
                            if(rows.length>0){
                                ...
                                if(val.properties.forceSet && obj.body[val.field]===val.properties.value){
                                    //console.log('TRUE');
                                    /*
                                     * Add field to new object that will be added to transactions
                                     * */
                                    $restfulize.helpers.insertFieldValue(val.field, val.properties.resetValue, objBuild.table, objTemp);
                                    $restfulize.helpers.pushTransaction("update", objTemp, obj);
                                ...
                                //self.core.errorHandler.error(409, 160, 'The following an $onlyOne error.');
                                //callback(true);
