...
module.exports = function(){
    ...
    this.$processUniqueDataFields = function(obj){
        try{
            ...
            var strFilter = "";
            var aryFilter = (obj.properties.filter).split(/({{|}})/).filter(function(x){ return x; });

            var lngAryFilter = aryFilter.length;
            var bolActiveTag = false;
            for(var i=0;i<lngAryFilter;i++){
                var item = aryFilter[i];
                switch(item){
                    case '{{':
                        bolActiveTag = true;
                        break;
                    ...
                    default:
                        if(bolActiveTag){
                            if(typeof obj.temp.body[item]!=='undefined'){
                                strFilter += obj.temp.body[item];
                            }else{
                                strFilter += item;
                            }
                        }else{
