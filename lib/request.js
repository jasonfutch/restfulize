...
module.exports = function restfulQuery(tables, req, res, errorHandler){
    ...
    this.build = function(type, obj){
        ...
        //set force
        if(o._force!==undefined) obj._force = o._force;
    ...
    this.$stringToObject = function(p, bolOnlySetValues){
        ...
        if(structure!==''){
            ...
            for(var i=0;i<lngStructures;i++){
                ...
                if(structureBin[binCountString]===undefined) structureBin[binCountString] = [];
            ...
            for(var i=structureBinLargest;i>=0;i--){
                if(structureBin[i+'']!==undefined) obj.structure = obj.structure.concat(structureBin[i+'']);
            }
        }
        ...
        if(aggregate!==''){
            obj.aggregates = [];
            var aryAggregates = self.$toArray(aggregate);
    ...
    this.$combineDefaults = function(obj,defaults){
        ...
        if(defaults.structure!=undefined){
    ...
    this.$validateAggregate = function (obj,fields){ //validate aggregate field
        ...
        for(var i=0;i<lngAggregate;i++){
            ...
            if(fields[field]===undefined) return self.onError(400,100,obj._sectionExt+'aggregate field:'+field+' was not found in the collection');
        }
        return true;
    };
