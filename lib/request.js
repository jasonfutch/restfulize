...
module.exports = function restfulQuery(tables, req, res, errorHandler){
    ...
    this.$stringToObject = function(p, bolOnlySetValues){
        ...
        (!_skipExistCheck) ? _skipExistCheck='' : _skipExistCheck = self.$removeAllSpaces(_skipExistCheck);
        ...
        (!namespace) ? namespace='' : namespace = self.$removeAllSpaces(namespace);
        ...
        (!structure) ? structure='' : structure = self.$removeAllSpaces(structure);
        if(structure!==''){
            obj.structure = [];
            ...
            for(var i=0;i<lngStructures;i++){
                var objStructure = {section:"", arySection:[], param:""};
                var aryStructure = self.$toArray(aryStructures[i],1);
            ...
            for(var i=structureBinLargest;i>=0;i--){
                if(typeof structureBin[i+'']!=='undefined') obj.structure = obj.structure.concat(structureBin[i+'']);
            }
        }

        //_compareKey
        var _compareKey = p[_section+'_compareKey'];
        (!_compareKey) ? _compareKey='' : _compareKey = self.$removeAllSpaces(_compareKey);
        if(_compareKey!==''){
            obj._compareKey = [];
            aryCompareKey = self.$toArray(_compareKey);
            lngCompareKey = aryCompareKey.length;
            for(var i=0;i<lngCompareKey;i++){
                objCompareKey = {name:""};
                objCompareKey.name = aryCompareKey[i];
    ...
    this.$convertFieldNames = function(obj,fields, bolRemoveDotNotation){
        ...
        for(var i=0;i<lngFields;i++){
            if(isObject){
                ...
                if(typeof objColumn==='undefined') console.log(objField.name+' could not be found');
    ...
    this.$getJoins = function(obj){
        ...
        * */
        ...
        for(var i=0;i<lngJoins;i++){
            ...
            if(typeof smartJoins==='undefined') {
    ...
    this.$getFields = function(obj,bolCustom){ //get fields from table using list
        ...
        if(typeof list!=='undefined' && !bolCustom){
            ...
            for(var i=0;i<lngList;i++){
                var field = list[i];
                if(typeof tableFields[field]!=='undefined') fields[field] = tableFields[field];
            }
        }else{
        ...
        for(var field in fields){
            if(fields.hasOwnProperty(field)){
                ...
                if(typeof fields[field]==='undefined'){
                    ...
                    console.log(obj.namespace + '.' + field);
                }else{
    ...
    this.$validateRequestedFields = function(obj,fields){ //validate requested fields
        ...
        for(var i=0;i<lngFields;i++){
            ...
            if(typeof fields[checkField]==='undefined') {
    ...
    this.$validateSort = function (obj,fields){ //validate sort fields
        ...
        for(var i=0;i<lngSort;i++){
            ...
            if(typeof fields[obj.namespace + '.' + field]!=='undefined'){
                ...
                return self.onError(400,100,obj._sectionExt+'sort field:\''+field+'\' was not found in the collection');
            }
        }
        return true;
    };
    ...
    this.$validateFilters = function(obj,fields,param){
        ...
        for(var i=0;i<lngFields;i++){
            ...
            if(typeof fields[obj.namespace+'.'+field]!=='undefined'){
                ...
                field = aryFilters.fields[i].name;
            }
            if(typeof fields[field]==='undefined') return self.onError(400,100,obj._sectionExt+'filters: \''+field+'\' was not found in the collection');
        }
