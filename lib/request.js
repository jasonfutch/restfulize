...
module.exports = function restfulQuery(tables, req, res, errorHandler){
            ...
            for (var i=0;i<lngFilters;i++){
                ...
                switch(filtr){
                    ...
                    default:
                        ...
                        var objArg = {name:aryArgs[0],value:"",sep:"=="};
                        if(aryArgs.length>1){
                            var value = aryArgs[1];
                            if(value!==''){
    ...
    this.$convertFieldNames = function(obj,fields){
        ...
        for(var i=0;i<lngFields;i++){
            if(isObject){
                objField = obj[i];
                objField.name = fields[objField.name].field;
            }else{
    ...
    this.$validateTables = function(obj){ //validates tables, joins & lists
        ...
        for(var i=0;i<lngJoins;i++){
            ...
            if(self.tables[obj.table.name].joins[join.name]===undefined) return self.onError(400,100,obj._sectionExt+'table:'+join.name+' is not allowed to join to '+obj.table.name);
            if(join.list!==''){
    ...
    this.$gatherFields = function(obj,bolGetAll){ //gather all fields from table and joins
        ...
        for(var i=0;i<lngJoins;i++){
            var join = obj.joins[i];
            if(join.list===''){
                join.list = self.tables[obj.table.name].joins[join.name].list;
            }
